...
namespace PriceOptimizerCoreApplication.web.Controllers
{
    public class ContactController : Controller
    {
        ...
        private string ControllerName = "ContactController";
        public ContactController(EmailHelper emailHelper, IRazorViewToStringRenderer razorViewToStringRenderer , Common common )
        {
        ...
        {
            var model = new Email(); 
            return PartialView("~/Views/Contact/_PartialContactUs.cshtml", model);
        }
        [HttpGet]
        ...
        [NoDirectAccess]
        public async Task<IActionResult> ContactUsAsync([Bind("To,Subject,Body,Name,RequestVerificationToken")] Email email)
        {
            try
            {
                ...
                return Json(new { Status = false, Message = "Please make sure all fields are filled in correctly..!", Html = await this.RenderViewToStringAsync("_PartialContactUs", email) });
            }
            catch (System.Exception ex)
            {
                var path = _common.GetFilePath(filePath);
                await Util.Utility.LogErrorMessage(ex, path,ControllerName,"ContactUsAsync");
                return Json(new { Status = false, Message = "There was an error.!" });
            }
